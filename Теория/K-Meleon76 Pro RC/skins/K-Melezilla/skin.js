pref("kmeleon.general.toolbars_locked", true);
pref("kmeleon.display.maximized", true);
pref("kmeleon.display.hideTitleBar", true);
pref("kmeleon.display.backgroundImageEnabled", true);

pref("kmeleon.toolband.KMrestart2.break", 1);
pref("kmeleon.toolband.KMrestart2.index", 0);
pref("kmeleon.toolband.KMrestart2.size", 36);
pref("kmeleon.toolband.KMrestart2.visibility", true);

pref("kmeleon.toolband.Menu.break", 0);
pref("kmeleon.toolband.Menu.index", 1);
pref("kmeleon.toolband.Menu.size", 466);
pref("kmeleon.toolband.Menu.visibility", true);

pref("kmeleon.toolband.Bookmarks.break", 0);
pref("kmeleon.toolband.Bookmarks.index", 2);
pref("kmeleon.toolband.Bookmarks.size", 600);
pref("kmeleon.toolband.Bookmarks.visibility", true);

pref("kmeleon.toolband.&Search Buttons.break", 0);
pref("kmeleon.toolband.&Search Buttons.index", 3);
pref("kmeleon.toolband.&Search Buttons.size", 31);
pref("kmeleon.toolband.&Search Buttons.visibility", true);

pref("kmeleon.toolband.&Search+.break", 0);
pref("kmeleon.toolband.&Search+.index", 3);
pref("kmeleon.toolband.&Search+.size", 36);
pref("kmeleon.toolband.&Search+.visibility", true);

pref("kmeleon.toolband.Extensions Manager.break", 0);
pref("kmeleon.toolband.Extensions Manager.index", 4);
pref("kmeleon.toolband.Extensions Manager.size", 76);
pref("kmeleon.toolband.Extensions Manager.visibility", true);

pref("kmeleon.toolband.AdBlock Plus (ABP7).break", 0);
pref("kmeleon.toolband.AdBlock Plus (ABP7).index", 5);
pref("kmeleon.toolband.AdBlock Plus (ABP7).size", 36);
pref("kmeleon.toolband.AdBlock Plus (ABP7).visibility", true);

pref("kmeleon.toolband.Winbuttons.break", 0);
pref("kmeleon.toolband.Winbuttons.index", 6);
pref("kmeleon.toolband.Winbuttons.size", 82);
pref("kmeleon.toolband.Winbuttons.visibility", true);

pref("kmeleon.toolband.&Main Bar.break", 1);
pref("kmeleon.toolband.&Main Bar.index", 7);
pref("kmeleon.toolband.&Main Bar.size", 207);
pref("kmeleon.toolband.&Main Bar.visibility", true);

pref("kmeleon.toolband.&Zoom Buttons.break", 0);
pref("kmeleon.toolband.&Zoom Buttons.index", 8);
pref("kmeleon.toolband.&Zoom Buttons.size", 75);
pref("kmeleon.toolband.&Zoom Buttons.visibility", true);

pref("kmeleon.toolband.URL Bar.break", 0);
pref("kmeleon.toolband.URL Bar.index", 9);
pref("kmeleon.toolband.URL Bar.size", 469);
pref("kmeleon.toolband.URL Bar.visibility", true);

pref("kmeleon.toolband.&Go Buttons.break", 0);
pref("kmeleon.toolband.&Go Buttons.index", 10);
pref("kmeleon.toolband.&Go Buttons.size", 75);
pref("kmeleon.toolband.&Go Buttons.visibility", true);

pref("kmeleon.toolband.SearchBar Ext.break", 0);
pref("kmeleon.toolband.SearchBar Ext.index", 11);
pref("kmeleon.toolband.SearchBar Ext.size", 253);
pref("kmeleon.toolband.SearchBar Ext.visibility", true);

pref("kmeleon.toolband.Throbber.break", 0);
pref("kmeleon.toolband.Throbber.index", 12);
pref("kmeleon.toolband.Throbber.size", 32);
pref("kmeleon.toolband.Throbber.visibility", false);

pref("kmeleon.toolband.Mail/&News Buttons.break", 0);
pref("kmeleon.toolband.Mail/&News Buttons.index", 13);
pref("kmeleon.toolband.Mail/&News Buttons.size", 94);
pref("kmeleon.toolband.Mail/&News Buttons.visibility", false);

pref("kmeleon.toolband.&Privacy Bar.break", 0);
pref("kmeleon.toolband.&Privacy Bar.index", 14);
pref("kmeleon.toolband.&Privacy Bar.size", 916);
pref("kmeleon.toolband.&Privacy Bar.visibility", false);

pref("kmeleon.toolband.Tabs.break", 1);
pref("kmeleon.toolband.Tabs.index", 15);
pref("kmeleon.toolband.Tabs.size", 1136);
pref("kmeleon.toolband.Tabs.visibility", true);

pref("kmeleon.toolband.Tab/&Window Buttons.break", 0);
pref("kmeleon.toolband.Tab/&Window Buttons.index", 16);
pref("kmeleon.toolband.Tab/&Window Buttons.size", 75);
pref("kmeleon.toolband.Tab/&Window Buttons.visibility", true);
